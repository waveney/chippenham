...
//    var_dump($_REQUEST);
  ...
  if (isset($_REQUEST['COPY'])) {
    $Pitches = Get_Trade_Pitches($loc,($_REQUEST['FY']??($YEAR-1)));
    foreach($Pitches as $Pitch) {
      $Pitch['Year'] = $PLANYEAR;
  ...
  echo "<h2>";
  if ($YEAR < $PLANYEAR) {
    echo "<a href=TradeSetup?COPY&l=$loc&FY=$YEAR>Copy to Current</a>,";
  } else {
    $LYear = $YEAR-1;
    echo "<a href=TradeSetup?COPY&l=$loc&FY=$LYear>Copy $LYear to Current</a>,";
  }
