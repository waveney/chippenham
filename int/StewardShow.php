<?php
  ...
  if ($ShowMode == 'HeaderFree') {
    dominimalhead("PA Requirements for " . $Ven['SN'],['cache/Style.css','css/festconstyle.css',"js/qrcode.js"]);
  } else {
  ...
  if (!$res || $res->num_rows==0) {
    echo "<h3>There are currently no events at " . $Ven['SN'] . " and hence no current Steward Requirements</h3>\n";
    dotail();
    exit;
  }
...
//  echo "<h2 class=FakeButton><a href=PAShow?pa4v=$V>Browsing Format</a>, <a href=PAShow?pa4v=$V&FILES=1>Embed Files</a>,  <a href=PAShow?pa4v=$V&FILES=2>Header Free for Printing</a></h2>";
  ...
  foreach ($EVs as $ei=>$e) {
    ...
    if ($e['SubEvent'] == 0) {
      ...
      if ($Ven['IsVirtual']) {
        $rows++;
        $str .= "<tr><td>Where:<td>" . $Venues[$e['Venue']]['SN'];
      }
...
//    if ($e['SubEvent']<1) $str .= "<tr><td class=ES_What>Price:<td class=ES_Detail>" . Price_Show($e,1);
  ...
  if (Access('Staff')) {
    ...
    echo "<h3>Link to send to Manager/Steward: $host/int/Access?Y=$YEAR&t=m&i=$V&k=" . $Ven['AccessKey'];
